...
# Start JSON array
echo "[" > src/data/allstar-cards.json
...
for card in "${cards[@]}"; do
  ...
  cat >> src/data/allstar-cards.json <<CARDEOF
  {
    "name": "$name",
    "image_uri": "$image_uri",
...
echo "Done! Saved $count cards to src/data/allstar-cards.json"
